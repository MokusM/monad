...
// Адреси контрактів
const SYMBIOTIC_ROUTER = utils.CONTRACTS.SYMBIOTIC_ROUTER;
...
async function swapMONtoUSDC(wallet, amountIn) {
  try {
    console.log(`\nStarting swap operation on Symbiotic DEX...`.magenta);
    ...
    // Схвалюємо WMON для Symbiotic Router
    const approveResult = await utils.approveToken(wallet, WMON_ADDRESS, amountIn, SYMBIOTIC_ROUTER);
    if (!approveResult) {
    ...
    // ABI для Symbiotic Router (спрощений)
    const routerAbi = [
      'function swapExactTokensForTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external returns (uint[] memory amounts)',
      'function getAmountsOut(uint amountIn, address[] calldata path) external view returns (uint[] memory amounts)'
    ];
    
    const routerContract = new ethers.Contract(SYMBIOTIC_ROUTER, routerAbi, wallet);
...
async function addLiquidity(wallet, tokenA, tokenB, amountA, amountB) {
  try {
    console.log(`\nStarting liquidity provision on Symbiotic DEX...`.magenta);
    ...
    // Схвалюємо обидва токени для Symbiotic Router
    const approveAResult = await utils.approveToken(wallet, tokenA.address, amountA, SYMBIOTIC_ROUTER);
    if (!approveAResult) {
    ...
    const approveBResult = await utils.approveToken(wallet, tokenB.address, amountB, SYMBIOTIC_ROUTER);
    if (!approveBResult) {
...
async function runSymbioticLiquidity(wallet, useDelay = true) {
  try {
    console.log(`\nStarting Symbiotic Liquidity module...`.magenta);
    ...
    console.error(`Error in Symbiotic Liquidity module: ${error.message}`.red);
    return false;
  }
}
...
async function runInteractiveLiquidity() {
  console.log('Symbiotic Liquidity Module'.green.bold);
  console.log('========================='.green);
